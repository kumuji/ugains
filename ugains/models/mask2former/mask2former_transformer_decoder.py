...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        inlier_out = {
            "pred_logits": predictions_class[-1][:, : self.num_queries, :],
            "pred_masks": predictions_mask[-1][:, : self.num_queries, :],
            "features": features[:, : self.num_queries, :],
            "aux_outputs": self._set_aux_loss_inlier(
    ...
    @torch.jit.unused
    def _set_aux_loss_inlier(self, outputs_class, outputs_seg_masks):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if self.mask_classification:
            return [
                {
                    "pred_logits": a[:, : self.num_queries, :],
                    "pred_masks": b[:, : self.num_queries, :],
                }
                for a, b in zip(outputs_class[:-1], outputs_seg_masks[:-1])
            ]
        else:
